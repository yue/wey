...
class SlackChannel extends Channel {
  constructor(account, type, channel) {
    ...
    switch (type) {
      case 'channel':
        ...
        this.mentions = channel.mention_count_display
        break
      case 'dm':
        this.isMember = channel.is_open
        if (channel.is_mpim) {
          ...
          this.isRead = channel.unread_count_display === 0
        } else {
          if (channel.user_id === account.currentUserId)
            this.name = `${this.name} (you)`
          const user = account.findUserById(channel.user_id)
          if (user)
            this.isAway = user.isAway
          this.mentions = channel.dm_count
        }
        break
    }
  ...
  async sendMessage(text, thread) {
    const options = {text, channel: this.id}
    if (thread)
      options.thread_ts = thread
    ...
    if (thread)
      this.dispatchSubMessage(message)
    else
  ...
  async readThread(thread) {
    // Read messages.
    const options = {channel: this.id, ts: thread, limit: 50}
    ...
    for (const m of smsgs)  // slack messages have async info.
      await m.fetchPendingInfo(this.account)
    // Do not show replies count.
    smsgs[0].isThreadParent = false
    return smsgs
  }
