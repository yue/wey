...
class SlackAccount extends Account {
  constructor(service, data, rtm, config=null) {
    ...
    this.rtm.on('im_open', this.joinChannel.bind(this))
  ...
  async ready(data) {
    ...
    const channels = await this.listChannels(options, filterChannel)
    this.channels = channels.filter((c) => c.type === 'channel')
    const dms = this.filterDMs(channels.filter((c) => c.type === 'dm'))
    // Do not show all dms, but update info for all of them.
    this.directMessages = dms.slice(0, 5)
    // Notify.
    this.setReadState(this.computeReadState())
    this.isReady = true
    this.onUpdateChannels.dispatch(this.channels)
    // Update channel info asyncronously for unread information.
  ...
  async newDirectMessage(event) {
    const dm = (await this.rtm.webClient.conversations.info({channel: event.channel})).channel
    const channel = new SlackChannel(this, dm)
    this.directMessages.unshift(channel)
    this.onUnreadDM.dispatch(0, channel)
  }
  ...
  filterDMs(dms) {
    ...
    return dms.map(addOrderHint).sort(compareHint)
  }
}
