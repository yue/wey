...
class SlackAccount extends Account {
  constructor(service, data, token, config=null) {
    ...
    const options = {
      tlsConfig: {handshakeTimeout: 4000},
  ...
  async dispatchMessage(event) {
    ...
    if (channel) {
      const isThread = event.previous_message && event.previous_message.thread_ts
      const isThreadParent = isThread && event.previous_message.thread_ts === event.previous_message.ts
      const thread = isThread ? channel.findThread(event.previous_message.thread_ts) : null
      switch (event.subtype) {
        case 'message_deleted':
          if (thread)
            thread.deleteMessage(event.deleted_ts, event.deleted_ts)
          if (!isThread || isThreadParent)
