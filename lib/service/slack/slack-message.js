...
const {parseEmoji, slackMarkdownToHtml} = require('./message-parser')
...
class SlackMessage extends Message {
  constructor(account, event) {
    ...
    if (event.edited && !this.isBot)
      this.isEdited = true
    if (event.reactions)
      this.reactions = event.reactions.map((r) => {
        const skin = r.name.indexOf('::')
        if (skin !== -1)
          r.name = r.name.substring(0, skin)
        return {
          name: r.name,
          count: r.count,
          content: parseEmoji(account, 16, r.name)
        }
      })
