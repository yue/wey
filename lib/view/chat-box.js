...
class ChatBox {
  ...
  async _getProfile(id, timestamp) {
    ...
    if (win) {
      ...
      if(this.messageList.type === 'channel'){
        new ProfileWindow(profile, this.messageList.openThread(profile.id))
      }else if(this.messageList.type === 'thread'){
        //new ProfileWindow(profile, this.messageList.openThread(id))
      }
    }
