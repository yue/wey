...
class ChatBox {
  ...
  unload() {
    if (this.subscription) {
      ...
      this.subscription.onModifySubMessage.detach()
      this.subscription = null
    }
    if (this.channel) {
  ...
  async loadChannel(channel, threadId=null) {
    this.unload()
    ...
    this.subscription = {
      ...
      onModifySubMessage: channel.onModifySubMessage.add(this.modifyMessage.bind(this)),
    }
    // Make sure messages are loaded before loading the view.
    this.channel.startReceiving()
    const messages = this.threadId ? await channel.readThread(this.threadId)
                                   : await channel.readMessages()
    this.messagesLoaded = false
    // Start showing the messages.
    if (channel === this.channel) {
      this.channelHeader.loadChannel(this.channel)
      this.replyBox.setVisible(true)
      // TODO Remember unsent messages.
      this.replyEntry.setText('')
      this.adjustEntryHeight()
      const html = pageTemplate({channel, messages})
      if (process.env.WEY_DEBUG === '1')
        fs.writeFileSync('page.html', html)
      this.browser.loadHTML(html, channel.account.url)
    }
  }
  ...
  openThread(threadId) {
    ...
    new ChatWindow(this.channel, threadId, this.view.getBounds())
  }
  ...
  handleEnter(replyEntry) {
    ...
    this.channel.sendMessage(message, this.threadId)
                .then((res) => {
                  replyEntry.setText('')
