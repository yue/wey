...
class ChannelItem {
  ...
  async getUsers(){
	  if(!this.hasGottenUsers){
		const slackChannel = (await this.parent.account.rtm.webClient.channels.info({channel: this.channel.id})).channel
		const userList = slackChannel.members
		for(let u of userList){
			let user = await this.parent.account.rtm.webClient.users.info({user: u})
			if(this.userArray.indexOf(user.user.profile.display_name) == -1)
				this.userArray.push(user.user.profile.display_name)
			let presence = await this.parent.account.rtm.webClient.users.getPresence({user: u})
  ...
  _drawUserList(view, painter, dirty){
	  for(let i = 0; i < this.userArray.length; ++i){ 
		...
		painter.drawText(this.userArray[i]+ ' ' + this.presenceArray[i].presence, bounds, {font: NAME_FONT})
		bounds.y += 25
	  }
  }
}
