...
class SlackService extends Service {
  ...
  loginWithToken(token) {
    ...
    rtm.once('unable_to_rtm_start', (error) => this.callback(error))
    rtm.once('authenticated', (data) => this.callback(null, new SlackAccount(this, data, rtm)))
    rtm.start()
  }

  createLoginWindow() {
    this.loginWindow = gui.Window.create({})
    this.loginWindow.setTitle('Sign in to Slack')
  ...
  async fetchSlackTokens(fetchButton) {
    ...
    if (teams.length == 0) {
      fetchButton.setTitle('Retry (No Slack tokens found)')
      return
    }
