...
class ImageStore {
  constructor() {
    ...
    setTimeout(this._clearCache.bind(this), 1000 * 3600)
  }

  async getImage(url, options = {}) {
    const hash = sha1sum(url)
    const imagePath = path.join(this.dir, hash + path.extname(url))
    const now = Date.now()
    try {
      ...
      await fs.writeFile(imagePath, (await axios.get(url, options)).data)
    }
    return imagePath
  }
