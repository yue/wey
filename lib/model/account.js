...
class Account {
  constructor(service, config) {
    ...
    this.onUpdateDirectMessages = new Signal()
  ...
  findChannelById(id) {
    ...
    if (!channel)
      channel = this.directMessages.find((c) => c.id == id)
    return channel
  }

  computeReadState() {
    const compute = (r, c) => { return r = c.isRead ? r : false }
    return this.channels.reduce(compute, this.directMessages.reduce(compute, true))
  }
