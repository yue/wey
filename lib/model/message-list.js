...
class MessageList {
  ...
  unload() {
    ...
    if (this.isReceiving)
      console.error(`${this.receiverCount} receivers leaked when unloading MessageList`)
  }
  ...
  modifyMessage(id, timestamp, modified) {
    ...
    if (i === -1)
      return
    this.messages[i] = modified
    this.onModifyMessage.dispatch(id, modified)
  }
  ...
  async sendMessage(message, thread=null) {
    throw new Error('Should be implemented by subclasses')
  }
